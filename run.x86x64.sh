...
docker image rm ${github_username}/compose-example:node

echo "Criando imagens e containers..."
docker-compose -p compose-example -f docker-compose-x86x64.yml build
echo "Executando docker-compose up..."
docker-compose -p compose-example -f docker-compose-x86x64.yml up -d
